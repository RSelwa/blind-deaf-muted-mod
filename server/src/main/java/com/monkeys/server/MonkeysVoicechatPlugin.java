 ...
 * from {@link MonkeysServer#onInitialize()}, because Simple Voice Chat constructs
 * this plugin itself (no-arg) and can't pass our manager in.
 */
public final class MonkeysVoicechatPlugin implements VoicechatPlugin {

    /** Set once at server-mod init, read later when voice events fire. */
    private static volatile RoleManager roles;

    /** Wire in the role store. Called from {@link MonkeysServer#onInitialize()}. */
